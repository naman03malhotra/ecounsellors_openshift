	...
	return $amt1;
		...
		echo '<table class="default table" style="font-size:14px;">
									...
									<thead>
								 	...
								 	$q6=query("SELECT * from meet where confirm='1' order by date desc,id desc");
								 	$count=1;
								 	$c=0;
								$app_cnt=0;
...
"href="time?email='.$row_c['emailid'].'" target="_blank">'.$cname.'</a></td>
								 		...
								 		if($tday==1)
								 		echo '<td class="tday">'.date("jS M'y D",$rq6['date']).'</td>';
								 	else
										...
										echo '<td><a href="https://ecounsellors.in/chatx/?sub='.$rq6['rand'].'" target="_blank" class="btn btn-primary">Admin Chat</a></td>';
										...
										echo '<td>'.date("jS M'y D",$rq6['bnk_date']).'</td>';
											...
											echo '<td><a class="btn btn-block" style="background-color:red;color:white;" href="re?id='.$idx.'" target="_blank">Action</a></td>';
											echo '<td>'.$rq6['why_cancel'].'</td>';
...
<script type="text/javascript">
	...
	document.getElementById('app_disp').innerHTML="Total Appointments Today="+app_cnt.value
